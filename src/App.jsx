...
function App() {
  ...
  return (
    <section className='w-[800px] mt-4 mx-auto py-4 flex flex-col items-center justify-center gap-4 rounded-xl bg-blue-500'>
      ...
      <div>
        <form onSubmit={handleSubmit} className='w-[700px] mb-6 flex items-center justify-center gap-6'>
      ...
      <ul className='mb-4 flex flex-col items-center gap-4'>
        {todos?.map(todo => (
          <li key={todo.id} className='w-[700px] px-6 py-2 font-bold flex items-center justify-between rounded-3xl text-black bg-white'>
            {editTodo.id === todo.id ? (
              <input
                className='w-[530px] input input-secondary italic input-sm px-2 bg-transparent'
                type="text"
                value={editTodo.text}
                onChange={(e) => setEditTodo({ ...editTodo, text: e.target.value })}
              />
            ) : (todo.text)}
            {editTodo.id === todo.id ? (<button onClick={() => handleEdit(todo.id, editTodo.text)} className='mx-1 py-2 px-3 rounded-md text-black bg-green-500'>
              <i class="fa-solid fa-floppy-disk text-md"></i>
            </button>) :
              (<button onClick={() => setEditTodo({ id: todo.id, text: todo.text })} className='mx-1 py-2 px-3 rounded-md text-black bg-yellow-500'>
                <i className="fa-solid fa-pen-to-square text-md"></i>
              </button>)}
            <button onClick={() => dispatch(delet(todo.id))} className='mx-1 py-2 px-3 rounded-md text-black bg-red-500'>
